...
describe('Amazon Echo NodeJS ToolKit', function () {
  ...
  describe('Echo Launch Request', function () {
    it('should successfully construct a valid launch response', function (done) {
      ...
      // Testing..
      alexa.sessionId.should.be.exactly(req.session.sessionId);
      alexa.userId.should.be.exactly(req.session.user.userId);
      ...
      alexa.response('Launch Response', {
        ...
        // Testing..
        res.version.should.be.exactly(req.version);
        res.sessionAttributes.should.eql(req.session.attributes);
  ...
  describe('Echo Intent Request', function () {
    it('should successfully construct a valid intent response', function (done) {
      ...
      alexa.sessionId.should.be.exactly(req.session.sessionId);
      alexa.userId.should.be.exactly(req.session.user.userId);
      alexa.intentName.should.be.exactly(req.request.intent.name);
      alexa.slots.should.eql(req.request.intent.slots);
